        ...
        <div class="w-full">
            <div class="border border-gray-200 rounded-b-lg" style="max-width: 400px;margin: 0 auto;">
                <div class="flex flex-col justify-center p-4 space-y-2">
                    <A href="/ui/audio/this_week" attr:class="w-full btn btn-lg btn-accent">今天 Today</A>
            ...
            <div class="border border-gray-200 rounded-b-lg">
                {move || {
                    if entries.is_empty() {
                        ...
                        let mut prev_event = None::<String>;
                        entries.iter().enumerate().map(|(index, entry)| {
                            ...
                            let date_header = if Some(entry.pub_date) != prev_date {
                                ...
                                Some(view! {
                                    <div class="flex items-center justify-between px-4 py-2 text-lg font-bold text-gray-800 bg-gray-200 border-b">
                                        <span>{entry.pub_date.format("%A, %B %e, %Y").to_string()}</span>
                                        <A href="/ui/videos/date" attr:class="btn btn-sm btn-ghost">
                                            {calendar_icon()}
                                        </A>
                                    </div>
                                })
                            ...
                            let event_header = if Some(entry.event.clone()) != prev_event {
                                prev_event = Some(entry.event.clone());
                                Some(view! {
                                    <h4 class="px-4 py-1 font-semibold text-gray-700 bg-gray-100 border-b text-md">
                                        <span class="mr-2">{entry.pub_date.format("%m.%d").to_string()}</span><span class="mr-2">{entry.event}</span><span class="mr-2">{entry.event_desc}</span>
                                    </h4>
                                })
                            } else {
                                None
                ...
                {move || {
                    ...
                    if loading.get() {
                        ...
                        if entries.is_empty() {
                                    ...
                                    <>
                                        <div class="flex justify-center">
                                            <div class="alert alert-info">
                                                ...
                                                <span>No audio entries for the selected date range.</span>
                                            </div>
                                        </div>
                                        {menu_view(date_map.get(), set_selected_date)}
                                    </>
                            ...
                            let prev_date = entries[entries.len()-1].pub_date - chrono::Duration::days(1);
                            let next_date = entries[0].pub_date + chrono::Duration::days(1);
                            let today = Utc::now().date_naive();
                            view!{
                                <>
                                    <div class="flex justify-center mb-4">
                                        <A href=format!("/ui/videos/{}", prev_date.format("%y%m%d")) attr:class="btn btn-lg btn-accent">
                                            Previous Day
                                        </A>
                                    </div>
                                    ...
                                    {if next_date <= today {
                                        view! {
                                            <div class="flex justify-center mt-4">
                                                <A href=format!("/ui/videos/{}", next_date.format("%y%m%d")) attr:class="btn btn-lg btn-accent">
                                                    Next Day
                                                </A>
                                            </div>
                                        }.into_any()
