                            ...
                            let date_header = if Some(entry.pub_date) != prev_date {
                                ...
                                Some(view! {
                                    <div id={format!("date-{}", entry.pub_date.format("%Y%m%d"))} class="flex items-center justify-between px-4 py-2 text-lg font-bold text-gray-800 bg-gray-200 border-b">
                                        ...
                                        <A href="/ui/audio/date" attr:class="btn btn-sm btn-ghost">
                                            {calendar_icon()}
                                        </A>
                                    </div>
                                })
...
pub fn AudioView() -> impl IntoView {
    ...
    Effect::new(move |_| {
        ...
        spawn_local(async move {
            match fetch_files("Music/ZSF/Chinese".to_string()).await {
                Ok(ch) => {
                            ...
                            view!{
                                <>
                                    <div class="flex justify-center mb-4">
                                        <A href=format!("/ui/audio/{}", prev_date.format("%y%m%d")) attr:class="btn btn-lg btn-accent">
                                            Previous Week
                                        </A>
                                    </div>
                                    ...
                                    {if next_date <= today {
                                        view! {
                                            <div class="flex justify-center mt-4">
                                                <A href=format!("/ui/audio/{}", next_date.format("%y%m%d")) attr:class="btn btn-lg btn-accent">
                                                    Next Week
                                                </A>
                                            </div>
                                        }.into_any()
